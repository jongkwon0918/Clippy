...
import { BsCheckLg, BsX, BsJournalText, BsPerson, BsArrowRepeat} from "react-icons/bs";
...
export const ReviewScreen: React.FC<ReviewScreenProps> = ({ result, context, teamMembers, onConfirm, onCancel, onRegenerate }) => {
    ...
    <div className="flex flex-col h-full max-h-[80vh]">
      <div className="p-6 border-b border-gray-200 flex justify-between items-center flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
            <button 
                ...
                className="flex items-center gap-1.5 text-xs bg-red-50 text-primary px-3 py-1.5 rounded-lg font-bold hover:bg-red-100 transition-colors"
                title="다시 분석"
            >
          ...
          <div className="space-y-3">
            {editedTasks.map((task) => (
              ...
              >
                ...
                >
                    {selectedTaskIds.has(task.id) && <BsCheckLg className="h-3.5 w-3.5 text-white" />}
                </div>
      ...
      <div className="p-4 border-t border-gray-200 bg-gray-50 flex gap-3 flex-shrink-0 rounded-b-2xl">
        ...
        >
            ...
            <span>{selectedTaskIds.size}개 항목 등록</span>
        </button>
      </div>
    </div>
  );
